...
export const Dapp = () => {
	...
	return (
		<div style={{ padding: '3rem 5rem' }}>
			<center>
			<h1 style={{ color:'white' }}>Blockchain Online Voting System</h1>
			<div>
				<h4  style={{ color:'white' }}>chairperson: {chairperson}</h4>
			</div>
			<Proposals proposals={proposals} voteProposal={voteProposal}  style={{ color:'white' }}/>
			<AddVoter  style={{ color:'white' }}
				addNewVoter={addNewVoter }
				setNewVoter={setNewVoter}
				newVoter={newVoter}
				newVoterStatus={newVoterStatus}
			/>
			<CheckVoterAddress  style={{ color:'white' }}
